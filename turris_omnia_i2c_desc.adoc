...
=== CMD_SET_WAKEUP
* Set board wakeup time after powered off by CMD_POWER_OFF
* Time is given in seconds after boot, the current time after boot (uptime) can be determined with CMD_GET_UPTIME_AND_WAKEUP
* If set to 0, disables wakeup
* Available only if POWEROFF_WAKEUP feature bit is set in CMD_GET_FEATURES
* Write only

* Example of setting wakeup time to 3600s after boot
** "i2cset 1 0x2A 0x22 0x10 0x0e 0x00 0x00 i"
*** 1 -> i2cbus number
*** 0x2A -> I2C address of the slave
*** 0x22 -> CMD_SET_WAKEUP command code
*** 0x10 0x0e 0x00 0x00 -> little endian value for 3600s = 0xe10
***	i -> block data type


=== CMD_GET_UPTIME_AND_WAKEUP
* Get board uptime and configured wakeup time
* Wakeup time can be configured with CMD_SET_WAKEUP
* Available only if POWEROFF_WAKEUP feature bit is set in CMD_GET_FEATURES
* Read only

* Example of reading uptime and configured wakeup time
** "i2ctransfer 1 w1@0x2A 0x23 r8"
*** 1 -> i2cbus number
*** w1@0x2A -> write 1 byte at I2C address of the slave
*** 0x23 -> CMD_GET_UPTIME_AND_WAKEUP command code
*** r8 -> read 8 bytes (4 bytes for uptime, 4 bytes for wakeup time, both in little endian)


=== CMD_POWER_OFF
* Power off the board (by powering off voltage all controllable voltage regulators)
* If wakeup time is configured, the board will be woken up at that time
* Available only if POWEROFF_WAKEUP feature bit is set in CMD_GET_FEATURES
* Write only

* Example of powering down the board
** "i2cset 1 0x2A 0x24 0xAD 0xDE 0x01 0x00 0x12 0x72 0x3B 0xBA i"
*** 1 -> i2cbus number
*** 0x2A -> I2C address of the slave
*** 0x24 -> CMD_POWER_OFF command code
*** 0xAD 0xDE -> 0xDEAD magic number in little endian
*** 0x01 0x00 -> 0x0001 flag (enable poweron with front button)
*** 0x12 0x72 0x3B 0xBA -> CRC32 for the request
***	i -> block data type
